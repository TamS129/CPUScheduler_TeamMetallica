...
&nbsp; &nbsp; &nbsp; &nbsp;The First Come, First Serve scheduling algorithm is a scheduling strategy that finishes processes as they appear in the ready queue. Each of the processes in our experiment arrive at the same time, so the first burst of each process is executed in sequential order. When a CPU burst is completed, it still has a I/O burst, it will be put into the I/O queue. Once the process has returned from the I/O burst, it is put back at the end of the ready queue to be processed.

&nbsp; &nbsp; &nbsp; &nbsp;This algorithm’s strengths are that it is easy to implement, it gives each process a fair chance to execute based on its arrival time, and it has predictability in the execution of processes. Because of this simplicity, it has very low overhead due to not requiring as many complex data structures to handle.

&nbsp; &nbsp; &nbsp; &nbsp;However, it tends to have poor average waiting time since long processes can cause a backup in the ready queue. This can hurt CPU utilization as many processes spend a lot of time waiting. Because the process is non-preemptive, each CPU burst must complete its execution before moving on to the next one. This can cause potential issues since the processes cannot be interrupted. If you want to interrupt or interact with the system, this can be problematic.
...
**However**, the SJF algorithm cannot be implemented at the level of CPU Scheduling because it's impossible to know the burst times of a process beforehand. Therefore in SJF, each process is assigned an estimated burst time approximated as an exponential average of measured lengths of previous CPU bursts. The scheduler then picks the process with the shortest burst time and executes it until the burst time is over. Then if processes have I/O bursts as well, the process is sent to an I/O wait queue where it has to wait out the duration of it's I/O burst before it can join the ready queue. This cycle is repeated for the remaining processes until they're all fully completed.
...
The **Priority Scheduling** Algorithm is a versatile and widely used scheduling algorithm where each process is assigned a priority level, and the CPU is allocated to the process with the highest priority. This algorithm is naturally preemptive because if a new process arrives with a higher priority than the currently running process, the CPU is preempted and reassigned to the new process. This ensures that higher-priority tasks are executed promptly as needed.
...
&nbsp; &nbsp; &nbsp; &nbsp; The Multilevel Feedback Queue scheduling algorithm is a scheduling strategy that moves processes to lower priority queues based on the current burst time of the process. This method utilizes three queues, and initially loads all of the processes into the first queue. The first queue will run the first CPU burst of each process for a given time quantum. If the process finishes its CPU burst, it starts its I/O burst and starts the next process's CPU burst. If the process does not finish its burst, it will be moved down to the second queue to be processed once queue one is empty. Processes in queue two will have a longer time quantum to run, and will be moved to queue three if it does not finish. All processes will go to the end of queue one when they return from an I/O burst.

&nbsp; &nbsp; &nbsp; &nbsp;This algorithm tends to be very effective as it can increase CPU utilization as shorter processes finish quickly and reduce wait times. This is beneficial to interactive and I/O intensive operations. This gives the system a lot of flexibility to handle processes in an efficient manner by promoting and demoting processes based on their CPU burst times. It also excels at being a very customizable model, as you can increase or decrease the number of queues and time quantum for each queue depending on the special needs of the system.

&nbsp; &nbsp; &nbsp; &nbsp; Some of the downsides of this algorithm are that it tends to have a lot of complexity compared to other scheduling algorithms. It can be very difficult to adjust the rules for promotion/demotion, as well as the number of queues and time quantums. The algorithm also introduces a fair level of overhead due to its frequent context switching and use of data structures.This also has a byproduct of having varying performances of different processes, and can potentially lead to starving a process. A processes is considered starving when it sits in a low priority queue and is continually preempted by higher-priority processes.
